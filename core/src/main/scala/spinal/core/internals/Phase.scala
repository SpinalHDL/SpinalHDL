...
/**
 * Phase that collects all BlackBoxes mixing in VivadoIpTcl and generates a consolidated TCL script.
 */
case class PhaseVivadoIpTcl(report: SpinalReport[_]) extends Phase {
  ...
  override def impl(pc: PhaseContext): Unit = {
    ...
    pc.walkComponents {
      case bb: BlackBox with VivadoIpTcl => blackBoxes += bb
      case _ =>
    }

    val targetDir = pc.config.ipTclTargetDirectory.getOrElse(pc.config.targetDirectory)
    val topName = pc.topLevel.definitionName
    ...
    if (!targetDirFile.exists()) {
      ...
      if (!targetDirFile.exists()) {
        throw new RuntimeException(
          s"Failed to create target directory '${targetDirFile.getAbsolutePath}' for Vivado IP TCL generation"
        )
      }
    }
    ...
    if (blackBoxes.isEmpty) {
      ...
      val tclFile = new File(targetDir, s"${topName}_ips.tcl")
      val deleted = tclFile.delete()
      if (!deleted && tclFile.exists()) {
        System.err.println(s"Warning: Failed to delete stale Vivado IP TCL file '${tclFile.getAbsolutePath}'")
      }
      return
    }

    val uniqueIps = blackBoxes.toSeq
      .groupBy(_.definitionName)
      .map { case (_, instances) => instances.head }
      .toSeq
      .sortBy(_.definitionName)

    if (pc.config.separateIpTcl) {
      // One file per IP
      uniqueIps.foreach { ip =>
        val tclFile = new File(targetDir, s"${ip.definitionName}.tcl")
        val writer = new PrintWriter(tclFile)

        try {
          writer.println(s"# Vivado IP generation script for ${ip.definitionName}")
          writer.println(s"# Generated by SpinalHDL PhaseVivadoIpTcl")
          writer.println()
          writer.println(ip.vivadoIpTcl())
        } finally {
          writer.close()
        }
        report.ipTclSourcesPaths += tclFile.getAbsolutePath
      }
    } else {
      // Consolidated file
      val tclFile = new File(targetDir, s"${topName}_ips.tcl")
      val writer = new PrintWriter(tclFile)
      try {
        writer.println(s"# Consolidated Vivado IP generation script for $topName")
        writer.println(s"# Generated by SpinalHDL PhaseVivadoIpTcl")
        writer.println()

        uniqueIps.foreach { ip =>
          writer.println(s"################################################################################")
          writer.println(s"# IP: ${ip.definitionName}")
          writer.println(s"################################################################################")
          writer.println(ip.vivadoIpTcl())
          writer.println()
        }
...
object SpinalVhdlBoot{
  ...
  def singleShot[T <: Component](config: SpinalConfig)(gen: => T): SpinalReport[T] = ScopeProperty.sandbox{
    ...
    phases += new PhaseVivadoIpTcl(report)
