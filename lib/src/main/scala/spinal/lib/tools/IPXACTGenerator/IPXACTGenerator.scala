...
class IPXACTGenerator[T <: Component](rtl: SpinalReport[T]) {
  ...
  def generateIPXACT2022Component(vendor: String = "SpinalHDL", version: String = "1.0"): Unit = {
    if (sourcesPaths.size > 1) {
      ...
      IPXACT2022ComponentGenerator.generate(toplevelVendor = vendor, toplevelName = toplevelName, module = toplevelModule, version = version, generatePath = parentPath)
    }
  }
  ...
  def generateIPXACTVivadoComponent(vendor: String = "SpinalHDL", version: String = "1.0"): Unit = {
    if (sourcesPaths.size > 1) {
      ...
      IPXACTVivadoComponentGenerator.generate(toplevelVendor = vendor, toplevelName = toplevelName, module = toplevelModule, version = version, generatePath = parentPath)
    }
  }
  ...
  def generateDesignView(vendor: String = "SpinalHDL", version: String = "1.0"): Unit = {
    if (sourcesPaths.size <= 1) {
      ...
      IPXACT2022ComponentGenerator.generate(toplevelVendor = vendor, toplevelName = toplevelName, module = toplevelModule, version = version, generateDesign = true, generatePath = parentPath)
    }
  }
}
