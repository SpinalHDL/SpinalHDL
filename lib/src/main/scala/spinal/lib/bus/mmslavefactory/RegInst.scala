...
case class RamInst(name: String, sizeMap: SizeMapping, MMSlaveFactory: MMSlaveFactory) {
  ...
  val hitRead  = hitRange(MMSlaveFactory.readAddress)
  val hitWrite = hitRange(MMSlaveFactory.writeAddress)
  val hitDoRead  = hitRead && MMSlaveFactory.doRead
  val hitDoWrite = hitWrite && MMSlaveFactory.doWrite
...
class FIFOInst(name: String, addr: Long, doc:String, MMSlaveFactory: MMSlaveFactory) extends RegBase(name,addr,doc,MMSlaveFactory) with FifoDescr {
...
case class RegInst(name: String, addr: Long, doc: String, MMSlaveFactory: MMSlaveFactory) extends RegBase(name, addr, doc, MMSlaveFactory) with RegDescr {
  def checkLast={
    val spareNumbers = if(fields.isEmpty) MMSlaveFactory.busDataWidth else MMSlaveFactory.busDataWidth-1 - fields.last.tailBitPos
    spareNumbers match {
      case x if x > 0 => field(x bits, AccessType.NA)(SymbolName("reserved"))
      case x if x < 0 => SpinalError(s"Range ${Section(fields.last.section)} exceed Bus width ${MMSlaveFactory.busDataWidth}")
      case _ =>
    }
  }
  ...
  def fieldAt(pos: Int, bc: BitCount, acc: AccessType, resetValue: Long = 0, doc: String = "")(implicit symbol: SymbolName): Bits = {
    ...
    val ret = pos match {
      ...
      case _ if sectionNext.max >= MMSlaveFactory.busDataWidth => SpinalError(s"Range ${sectionNext} exceed Bus width ${MMSlaveFactory.busDataWidth}")
...
abstract class RegBase(name: String, addr: Long, doc: String, MMSlaveFactory: MMSlaveFactory) {
  ...
  val hitRead  = MMSlaveFactory.readAddress === U(addr)
  val hitWrite = MMSlaveFactory.writeAddress === U(addr)
  val hitDoRead  = hitRead && MMSlaveFactory.doRead
  val hitDoWrite = hitWrite && MMSlaveFactory.doWrite
  ...
  protected def W1(bc: BitCount, section: Range, resetValue: Long ): Bits ={
    ...
    when(hitDoWrite && hardRestFirstFlag){
      ret := MMSlaveFactory.writeData(section)
  ...
  protected def WB(section: Range, resetValue: Long, accType: AccessType): Bits = {
    ...
    when(hitDoWrite){
      for(x <- section) {
        ...
        accType match {
          case AccessType.W1C => when( MMSlaveFactory.writeData(x)){ret(idx).clear()}
          case AccessType.W1S => when( MMSlaveFactory.writeData(x)){ret(idx).set()  }
          case AccessType.W1T => when( MMSlaveFactory.writeData(x)){ret(idx) := ~ret(idx)}
          case AccessType.W0C => when(~MMSlaveFactory.writeData(x)){ret(idx).clear()}
          case AccessType.W0S => when(~MMSlaveFactory.writeData(x)){ret(idx).set()  }
          case AccessType.W0T => when(~MMSlaveFactory.writeData(x)){ret(idx) := ~ret(idx)}
          case _ =>
        }
      }
    }
    ...
    for(x <- section) {
      ...
      accType match {
        case AccessType.W1SRC => {
          when(hitDoWrite && MMSlaveFactory.writeData(x)) {ret(idx).set()}
            .elsewhen(hitDoRead)                 {ret(idx).clear()}
        }
        case AccessType.W1CRS => {
          when(hitDoWrite && MMSlaveFactory.writeData(x)) {ret(idx).clear()}
  ...
  protected def WBP(section: Range, resetValue: Long, accType: AccessType): Bits ={
    ...
    for(x <- section) {
      ...
      accType match {
        case AccessType.W1P => {
          when(hitDoWrite &&  MMSlaveFactory.writeData(x)){ret(idx) := ~ret(idx)}
