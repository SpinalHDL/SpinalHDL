...
class AvalonUart( uartCtrlConfig : UartCtrlGenerics) extends Component{
  ...
  busCtrl.drive(uartCtrl.io.config.clockDivider,address = 0)
  busCtrl.drive(uartCtrl.io.config.frame,address = 4)
  busCtrl.createFlow(Bits(uartCtrlConfig.dataWidthMax bits),8).toStream >-> uartCtrl.io.write
  busCtrl.readStreamNonBlocking(uartCtrl.io.read.toStream.queue(64),8)
