...
//  }
//
//  def onPutPartialData(debugId : Long,
//                       source : Int,
//                       address : Long,
//                       size : Int,
//                       mask : Array[Boolean],
...
//          onPutPartialData(debugId, source, address, size, p.mask.toBytes.flatMap(v => (0 to 7).map(i => ((v>>i)&1).toBoolean)).drop(offset), p.data.toBytes.drop(offset))
//        }
//      }
//    }
//
