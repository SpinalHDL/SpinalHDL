...
/**
  * Definition of the Write address channel
...
case class AxiLiteB(config: AxiLiteConfig) extends Bundle {
  ...
  def setOKAY()   : Unit = resp := AxiLiteCst.rest.OKAY
  def setEXOKAY() : Unit = resp := AxiLiteCst.rest.EXOKAY
  def setSLVERR() : Unit = resp := AxiLiteCst.rest.SLVERR
  def setDECERR() : Unit = resp := AxiLiteCst.rest.DECERR
}


/**
  * Definition of the Read address channel
  * @param config Axi Lite configuration class
  */
