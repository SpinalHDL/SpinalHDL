...
final case class CHeaderGenerator(
    ...
    def end() : Unit = {
        ...
        implicit class RegDescrCheadExtend(reg: RegDescr) {
            def define(maxlen: Int = 40): String  = {
                val fdmaxlens = reg.fdnamelens
                val _tab = " " * (maxlen - reg.getName().size)
                s"""#define ${reg.getName().toUpperCase()} ${_tab}0x${reg.getAddr().hexString(16)}${fddefine(maxlen + fdmaxlens)}""".stripMargin
            }
        ...
        def body() = {
            ...
            s"""|/*
                ...
                |#define ${guardName}
                |
                |${regs.map(_.define(maxnamelen)).mkString("\n")}
                |
