...
case class DelayCyc(config: DfiConfig, timeConfig: DfiTimeConfig) {
  ...
  def sp2np(startphase: UInt, delaycyc: Int): UInt = (startphase + delaycyc).resize(log2Up(config.frequencyRatio))
  def sp2np(startphase: Int, delaycyc: Int): Int = (startphase + delaycyc) % config.frequencyRatio
  def mcdelaycyc(startphase: UInt, phydelaycyc: Int): UInt = (phydelaycyc) >> log2Up(config.frequencyRatio)
