...
//    idelay.CE := False
//
//    val clkin = (idelay.DATAOUT)
////    val clkin = BUFIO.on(idelay.DATAOUT)
////    val clkin = serdesClk90.readClockWire
////    val clkin = BUFG.on(idelay.DATAOUT)
////    val clkin = BUFIO.on(buf.O)
//  }
//
//  val dm = for(i <- 0 until (sl.dataWidth+7)/8) yield new Area{
//    io.sdram.DM(i) := ddrToOutput("DM", dmReg.map(_.map(_(i))))
...
//    des.CE1 := True
//    des.CE2 := True
//    des.CLK := dqs(i/8).clkin
//    des.CLKB := !dqs(i/8).clkin
//    des.CLKDIV := ClockDomain.current.readClockWire
//    des.CLKDIVP := False
//    des.D := False
//    des.DDLY := idelay.DATAOUT
