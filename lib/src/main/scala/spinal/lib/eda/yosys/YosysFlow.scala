...
object YosysFlow {
  def ice40Flow[T <: Component](report: SpinalReport[T]) = Yosys.load_SystemVerilog(report) + Yosys.synthesize("ice40") |> NextPNR_ice40()

  def formalFlow[T <: Component](report: SpinalReport[T],mode: String = Mode.bmc, multiclock: Boolean = false,memoryMap: Boolean= false) = InputFile(report) |> Yosys.formal(report,mode,multiclock,memoryMap) |> FormalCommand()
}
