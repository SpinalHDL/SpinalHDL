...
object DmaSg{
  ...
  def getSgReadRequirements(p : Parameter) = BmbAccessParameter(
    addressWidth = p.sgReadAddressWidth,
  ...
  case class Core[CTRL <: Data with IMasterSlave](p : Parameter, ctrlType : HardType[CTRL], slaveFactory : CTRL => BusSlaveFactory) extends Component{
    ...
    class ChannelLogic(val id : Int) extends Area{
      ...
      val push = new Area{
        ...
        val s2b = cp.inputsPorts.nonEmpty generate new Area {
          ...
          val packetLock = Reg(Bool)
          fifo.pop.withOverride.load setWhen( packetEvent )
    ...
    val s2b = for(portId <- 0 until p.inputs.size) yield new Area{
      ...
      val cmd = new Area {
        val channelsOh = B(channels.map(c => c.channelValid && !c.push.memory && c.push.s2b.portId === portId && c.push.s2b.sinkId === sink.sink))
        ...
        for ((channel, ohId) <- channels.zipWithIndex) {
          ...
          channel.push.s2b.packetLock setWhen(hit && sink.last)
        }
        when(sink.fire){
          when(noHit){
    ...
    val driver = StreamDriver(inputsIo(inputId), clockDomain) { p =>
      if (packets.isEmpty) {
        ...
        if(packet.data.isEmpty && Random.nextBoolean()) {
          ...
          if(packet.next != null) packets += packet.next
        } else {
          p.last #= false
        }
        true
    ...
    for (r <- 0 until 1000) {
      ...
      tests.randomPick() match {
        case S2M => if (p.inputs.nonEmpty) {
          val TRANSFER, PACKETS_SELF, PACKETS_STOP = new Object
