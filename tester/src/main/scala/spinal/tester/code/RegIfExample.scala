...
object PrettyPrinter extends BusIfVisitor {
  def begin(busDataWidth: Int): Unit = {
    println("---")
    println(s"Bus width = ${busDataWidth}")
  }

  def reg(name: String, addr : Long): Unit = {
    println(s"REG 0x${addr.toHexString}: ${name}")
  }
...
case class CHeaderGenerator(prefix : String) extends BusIfVisitor {
  var guardName : String = s"${prefix}_REGIF_H"
  ...
  def begin(busDataWidth: Int): Unit = {
    ...
    |//
    |""".stripMargin)
    println(s"#ifndef ${guardName}")
    println(s"#define ${guardName}\n")
  }

  def reg(name: String, addr : Long): Unit = {
    println(s"#define ${prefix}_${name.toUpperCase()} 0x${addr.toHexString}")
  }
...
object getRegIfExample {
  def main(args: Array[String]) {
    ...
    example.toplevel.busif.accept(PrettyPrinter)
    example.toplevel.busif.accept(CHeaderGenerator("AP"))
  }
}
