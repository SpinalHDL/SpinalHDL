  ...
  test("mix of slicing operations") {
    SpinalFormalConfig(_keepDebugInfo = true)
      ...
      .doVerify(new Component {
        val dut_8_2bit = FormalDut(SliceDutBits(8, 2 bit)).formalAsserts() // default case
        val dut_8_5bit = FormalDut(SliceDutBits(8, 5 bit)).formalAsserts() // uneven case, https://github.com/SpinalHDL/SpinalHDL/issues/1049
        val dut_9_4bit = FormalDut(SliceDutBits(9, 4 bit)).formalAsserts() // odd width
        val dut_2_3bit = FormalDut(SliceDutBits(2, 3 bit)).formalAsserts() // width smaller than sliceWidth
        val dut_8_2slices = FormalDut(SliceDutSlices(8, 2 slices)).formalAsserts() // default case
        val dut_8_3slices = FormalDut(SliceDutSlices(8, 3 slices)).formalAsserts() // uneven case
        val dut_9_4slices = FormalDut(SliceDutSlices(9, 4 slices)).formalAsserts() // odd width
