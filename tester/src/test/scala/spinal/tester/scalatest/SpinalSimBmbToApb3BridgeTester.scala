...
class SpinalSimBmbToApb3BridgeTester extends FunSuite{
  SpinalSimTester { env =>
    import env._
    test(prefix + "test1") {
      ...
      SimConfig.compile {
        ...
        Phase.setup {
          ...
          val retainers = List.fill(1 << dut.bmbParameter.sourceWidth)(Phase.stimulus.retainer((10000*durationFactor).toInt))
