...
object SpinalSimBmbSmpInterconnectGeneratorTester {
  def f() = {
    new GeneratorComponent(new Generator {
      val interconnect = BmbSmpInterconnectGenerator()
...
class SpinalSimBmbSmpInterconnectGeneratorTester  extends SpinalSimFunSuite{

  test("test1") {
    SimConfig.allOptimisation.compile(SpinalSimBmbSmpInterconnectGeneratorTester.f).doSimUntilVoid("test1", 42) { dut => //TODO remove seed
      Phase.boot()
      Phase.setup {
