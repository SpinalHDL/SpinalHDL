...
class DfiControllerTester extends SpinalAnyFunSuite{
  ...
  test("DfiControllerTester"){
    SimConfig.compile{
      ...
      def write(array: Array[Int], address:BigInt=BigInt(ctp.port.bmb.access.addressWidth - log2Up(tpa.tp.bytePerTaskMax), simRandom) << log2Up(tpa.tp.bytePerTaskMax)) = {
      ...
      def read(beatCount:Int, address:BigInt=BigInt(ctp.port.bmb.access.addressWidth - log2Up(tpa.tp.bytePerTaskMax), simRandom) << log2Up(tpa.tp.bytePerTaskMax)): Unit = {
        io.bmb.cmd.address #= address
