...
class SimVerilator(backend : VerilatorBackend, handle : Long) extends SimRaw(){
  override def getInt(signal : Signal) : Int = {
    ...
    signal.dataType.raw64ToInt(backend.nativeInstance.wrapperGetU64(handle, signal.id), signal : Signal)
  }
  override def getLong(signal : Signal) : Long = {
    assert(signal.id != -1, "You can't access this signal in the simulation, as it isn't public")
    signal.dataType.raw64ToLong(backend.nativeInstance.wrapperGetU64(handle, signal.id), signal : Signal)
  }
  override def setLong(signal : Signal, value : Long) : Unit = {
    assert(signal.id != -1, "You can't access this signal in the simulation, as it isn't public")
    backend.nativeInstance.wrapperSetU64(handle, signal.id, signal.dataType.longToRaw64(value, signal : Signal))
  }
  ...
  override def getBigInt(signal: Signal) = {
    if(signal.dataType.width < 64 || (signal.dataType.width == 64 && signal.dataType.isInstanceOf[SIntDataType])) {
      ...
      val rawValue = backend.nativeInstance.wrapperGetU64(handle, signal.id)
      if(rawValue >= 0 ) {
        BigInt(rawValue)
      }else{
      ...
      if(signal.dataType.isInstanceOf[SIntDataType]){
        ...
        backend.nativeInstance.wrapperGetAU8(handle, signal.id, array)
        BigInt(array)
      }else{
  ...
  override def setBigInt(signal: Signal, value: BigInt): Unit = {
    ...
    if(valueBitLength <= 63) {
      ...
      backend.nativeInstance.wrapperSetU64(handle, signal.id, valueLong)
    } else {
      signal.dataType.checkBigIntRange(value, signal)
      val array = value.toByteArray
      backend.nativeInstance.wrapperSetAU8(handle, signal.id, array, array.length)
    }
  }

  override def eval() = backend.nativeInstance.wrapperEval(handle)
  override def sleep(cycles : Long) = backend.nativeInstance.wrapperSleep(handle, cycles)
  override def end() = backend.nativeInstance.wrapperDeleteHandle(handle)
